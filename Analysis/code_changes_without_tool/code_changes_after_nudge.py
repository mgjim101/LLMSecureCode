#!/usr/bin/env python3
"""
Count code changes after seeing the nudge (event 1) without necessarily running the tool (event 3).

For each participant in outputs/prolific_ids_with_counts.txt, find task pairs with both eventid=1
and eventid=3 in Tool_CSV/code_snapshots.csv, compare the code strings, and report whether the
participant changed their code after the nudge.

Output format mirrors outputs/code_changes_after_tool.txt.
"""
...
OUTPUT_PATH = SCRIPT_DIR / "code_changes_after_nudge.txt"
TASKS_CSV_PATH = SCRIPT_DIR / "code_changes_after_nudge_tasks.csv"
SUMMARY_CSV_PATH = SCRIPT_DIR / "code_changes_after_nudge_summary.csv"
PARTICIPANTS_CSV = BASE_DIR / "Tool_CSV" / "participants.csv"
COUNTS_CSV_PATH = SCRIPT_DIR / "code_changes_after_nudge_nudge_counts.csv"
        ...
        per_participant.setdefault(pid, []).append(
            {
                ...
                "start_timestamp": events["1"][0],
                "end_timestamp": events["3"][0],
            }
        )
...
def format_output(
    ...
    if nudge_mapping is None:
        nudge_mapping = {}
    lines: List[str] = []
    lines.append("Code changes after nudge only (event 1 vs event 3)")
    lines.append(f"Participants in scope: {len(participant_ids)} (from prolific_ids_with_counts.txt)")
    lines.append("Comparison rule: eventid=1 code is start (nudge seen), eventid=3 code is after.")
    lines.append("")
    ...
    for pid in participant_ids:
        ...
        lines.append(f"  Changed after nudge: {changed_count}")
        lines.append(f"  No change after nudge: {unchanged_count}")
        ...
        if tasks:
            ...
            lines.append("  Task details: none (no event 1 & 3 pair found)")
    ...
    lines.append(f"  Incomplete pairs skipped (missing event 1 or 3): {skipped_count}")
...
def main() -> None:
    ...
    print(f"Wrote code change summary (nudge only) to {OUTPUT_PATH}")
